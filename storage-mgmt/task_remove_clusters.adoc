...
[NOTE]
====
Debes eliminar un clúster de Unified Manager antes de decomisionarlo. Si el clúster ya está fuera de línea o no es accesible cuando intentas eliminarlo, Unified Manager no puede comunicarse con el clúster para eliminar el registro de la aplicación y las suscripciones configuradas al Event Management System (EMS). Esto hará que el proceso de eliminación falle o deje de responder antes de completarse.
