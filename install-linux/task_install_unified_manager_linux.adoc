...
`java -version`
 ...
 a| 
MySQL 8.4.7 Edición Comunitaria
 a| 
`rpm -qa | grep -i mysql`
...
|===
. Si la versión instalada de MySQL es anterior a MySQL 8.4.7 Community Edition, ingrese el siguiente comando para desinstalarla:
+
...
+
Para un sistema Red Hat Enterprise Linux 8, si tiene repositorios internos con JDK 17 (OpenJDK, 7-zip y otros paquetes de software proporcionados por el repositorio AppStream), debe deshabilitar su repositorio AppStream e instalar MySQL Community Server. Ejecute el siguiente comando:
